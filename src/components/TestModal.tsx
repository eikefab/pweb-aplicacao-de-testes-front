...
export default function TestModal({ isOpen, onClose, onSuccess }: TestModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-slate-800/95 border border-slate-700 rounded-xl p-8 shadow-2xl">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm">Preencha os dados do novo teste</p>
        </div>

        {(validationError || createTestMutation.isError) && (
          <div className="mb-4 p-3 bg-red-500/20 border border-red-500 rounded-lg text-red-200 text-sm">
            {validationError || 
              (createTestMutation.error instanceof ApiError 
                ? createTestMutation.error.message 
                : 'Falha ao criar teste. Tente novamente.')}
          </div>
        )}
